...
const QuestionEditor = (props: QuestionEditorProps) => {
    ...
    const renderReadOnly = () => (
        <div className="space-y-5">
                ...
                <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                        {formOptions.map((opt, idx) => {
                            ...
                            return (
                                <div key={idx} className={`rounded-xl border p-3 flex items-start gap-3 ${isCorrect ? "border-green-500/40 bg-green-500/5" : "border-border bg-muted/10"}`}>
                                    <span className={`text-xs font-bold mt-0.5 shrink-0 ${isCorrect ? "text-green-600" : "text-muted-foreground"}`}>
                                        {String.fromCharCode(65 + idx)}.
                                    </span>
                                    <div className="flex-1 min-w-0">
                                        {hasContent ? <HtmlView html={opt} className="text-sm break-words" /> : <span className="text-sm">{opt}</span>}
                                    </div>
                                    {isCorrect ? <Badge className="rounded-full text-[10px] bg-green-600 shrink-0"><CheckCircle2 className="h-3 w-3 mr-1" /> Correct</Badge> : null}
                                </div>
                            );
                        })}
                    </div>
            ...
            {/* Reference answer for subjective */}
            {(formQuestionType === "SHORT_ANSWER" || formQuestionType === "UPLOAD") && formReferenceAnswer && (
                <div className="space-y-2">
                    <Label className="text-muted-foreground">Reference Answer</Label>
                    <div className="rounded-xl border border-border bg-muted/10 p-4">
                        <p className="text-sm whitespace-pre-wrap">{formReferenceAnswer}</p>
                    </div>
                </div>
            )}
    ...
    const renderEditor = () => (
        <>
            <div className="space-y-5">
                ...
                {/* UPLOAD: Reference answer (text and/or image) + instructions */}
...
// ---------------------------------------------------------------------------
// Upload Reference Section — extracted for cleaner code
// ---------------------------------------------------------------------------
